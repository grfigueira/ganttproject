...
class UserStorySaver extends SaverBase {
    void save(IGanttProject project, TransformerHandler handler) throws SAXException {
        final AttributesImpl attrs = new AttributesImpl();
        startElement("userStories", handler);
        saveCustomColumnDefinitions(project, handler);
        UserStory[] userStories = project.getUserStoryManager().getUserStoriesArray();
        for (int i = 0; i < userStories.length; i++) {
            UserStory p = userStories[i];
            addAttribute("id", p.getId(), attrs);
            addAttribute("name", p.getName(), attrs);
            addAttribute("story", p.getStory(), attrs);
            startElement("userStory", attrs, handler);
            {
                //saveRates(p, handler);
                saveCustomProperties(project, p, handler);
            }
            endElement("userStory", handler);
        }
        endElement("userStories", handler);
    }
    ...
    private void saveCustomProperties(IGanttProject project, UserStory userStory, TransformerHandler handler)
            throws SAXException {
        // CustomPropertyManager customPropsManager =
        // project.getHumanResourceManager().getCustomPropertyManager();
        AttributesImpl attrs = new AttributesImpl();
        List<CustomProperty> properties = userStory.getCustomProperties();
    ...
    private void saveCustomColumnDefinitions(IGanttProject project, TransformerHandler handler) throws SAXException {
        CustomPropertyManager customPropsManager = project.getUserStoryManager().getCustomPropertyManager();
