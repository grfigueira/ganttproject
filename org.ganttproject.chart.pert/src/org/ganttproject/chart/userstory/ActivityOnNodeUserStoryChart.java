...
import org.ganttproject.chart.userstory.UserStoryChartAbstraction.TaskGraphNode;
 ...
 *
 */
public class ActivityOnNodeUserStoryChart extends UserStoryChart {
  ...
  private UserStoryChartAbstraction myPertAbstraction;
  ...
  public ActivityOnNodeUserStoryChart() {
      ...
      public void mouseDragged(final MouseEvent e) {
        if (myPressedGraphicalNode != null) {
          ...
          if (e.getX() > getPreferredSize().getWidth()) {
            ActivityOnNodeUserStoryChart.this.setPreferredSize(new Dimension(myPressedGraphicalNode.x + getNodeWidth() + getxGap(),
                (int) getPreferredSize().getHeight()));
            revalidate();
          }
          if (e.getY() > getPreferredSize().getHeight()) {
            ActivityOnNodeUserStoryChart.this.setPreferredSize(new Dimension((int) getPreferredSize().getWidth(),
  ...
  protected void buildPertChart() {
    if (myPertAbstraction == null) {
      myPertAbstraction = new UserStoryChartAbstraction(myTaskManager);
  ...
  private class GraphicalNode extends JComponent {
    ...
    private void paintMe(Graphics g) {
      ...
      Color color;
      switch (type) {
      case UserStoryChartAbstraction.Type.NORMAL:
        color = NORMAL_COLOR;
        break;
      case UserStoryChartAbstraction.Type.SUPER:
        color = SUPER_COLOR;
        break;
      case UserStoryChartAbstraction.Type.MILESTONE:
